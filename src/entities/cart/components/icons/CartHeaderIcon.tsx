...
export const CartHeaderIcon: FC<CartProps> = ({
  ...
  const counterSize = useMemo(() => getCircleSize(counter), [counter]);
  const additionalSize = useMemo(
    () => (counterSize === CircleSizes.Large ? LARGE_CIRCLE_ADD_WIDTH : 0),
    [counterSize]
  );
    ...
    <div className={cn(containerClassName, "relative")}>
      ...
      >
        ...
        <path
          ...
          fill="white"
        />
        {counterSize === CircleSizes.Small && (
          <circle cx="40" cy="11" r="8" fill="#F20000" />
        )}
        {counterSize === CircleSizes.Medium && (
          <ellipse cx="40" cy="12" rx="11" ry="8" fill="#F20000" />
        )}
        {counterSize === CircleSizes.Large && (
      ...
      {counterSize && (
        <span
          className={cn(
            ...
            styles.cartIconText,
            styles[`cartIconText-${counterSize}`]
          )}
        >
          {counterSize === CircleSizes.Large ? "99+" : counter}
        </span>
      )}
    </div>
  );
